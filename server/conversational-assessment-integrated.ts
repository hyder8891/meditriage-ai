...
`;
  ...
  // NOTE: Healthcare provider recommendation is now displayed ONLY in the structured
  // AssessmentResultCard component via resourceMatch field, NOT in the chat message.
  // This prevents clinic names from appearing as text in the chat bubble.
...
*⚕️ تنبيه: هذا التقييم لا يغني عن استشارة الطبيب المتخصص. يرجى مراجعة طبيب للحصول على التشخيص والعلاج المناسب.*`;
  ...
  return {
    ...
    evidence: [], // Literature citations removed per user request
